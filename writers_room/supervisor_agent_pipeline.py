...
context_aware_critique_agent = prelude | critique_agent
critique_taking_node = functools.partial(
    agent_node, agent=context_aware_critique_agent, name="Critique Agent"
)

content_repurposing_agent = create_agent(
    llm,
    [repurpose_content_to_tweets],
    "You are an expert senior researcher tasked with re-purposing content",
)
...
draft_writing_supervisor = create_team_supervisor(
    ...
    " task and respond with their results and status. When finished,"
    " respond with FINISH.",
    ["Draft Writer", "Critique Agent", "Repurposing Agent"],
)

authoring_graph = StateGraph(WritingTeamState)
authoring_graph.add_node("Draft Writer", draft_writing_node)
authoring_graph.add_node("Critique Agent", critique_taking_node)
...
authoring_graph.add_edge("Critique Agent", "supervisor")
    ...
    for s in super_graph.stream(
        {
            "messages": [
                HumanMessage(
                    content=f"Research the following topic. Once you have the reearch information, you should pass it off to the writing team \n TOPIC: {user_input}",
                )
            ],
        },
